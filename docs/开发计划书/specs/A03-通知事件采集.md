...
- `[已确认]` 在复制标题、正文和动作前用 Android 通知类别、媒体会话标记和 ongoing 标记派生五类通知，并应用用户类型开关；媒体播放控件默认关闭，其余类型默认开启。
...
| `MonitoringPolicySnapshot` | 原子只读采集策略，包含自动排除、用户排除、允许记录的通知类型和 `policyRevision` |
...
5. `[已确认]` 来源未排除时，只读取 `category`、是否含媒体会话标记和 ongoing 标记，按固定优先级派生 `MEDIA`、`COMMUNICATION`、`ALERT_OR_NAVIGATION`、`ONGOING_OR_PROGRESS`、`OTHER`；命中关闭类型时输出 `Excluded(TYPE_POLICY)`，不得读取可见文本或动作。
6. `[已确认]` 应用未声明或无法识别类别时归入 `OTHER`，不根据标题、正文或包名猜测类型。
7. `[候选]` 对 `REMOVED` 只复制身份、时间和移除原因，`content` 为空且 `runtimeAction` 为空。
8. `[候选]` 对 `POST_OR_UPDATE` 和 `RECOVERY_SNAPSHOT` 在回调内逐字段把允许的可见 `CharSequence` 安全转换成独立 `String`；某个可选字段失败则置空并记录对应 `SnapshotCopyWarning`，不牺牲其余字段。随后复制动作能力和当前进程 `runtimeAction` 引用；不复制大图、位图、完整 extras 或任意 `Intent`。
9. `[候选]` 构造不可变 `NotificationSnapshot`，清除所有 framework 容器引用；调度器立即投影纯数据 `NotificationObservation`，并把 `runtimeAction` 仅随当前 `observationId` 暂存在内存，绝不传给 A04/A05。
10. `[已确认]` 调度器不得静默丢弃。入队失败必须输出 `Failed(INGESTION_BACKPRESSURE)`；队列容量和退让方式由 Phase 0 锁定。
11. `[候选]` 生成 `NOTIFICATION_CALLBACK` 健康证据候选；只有与通知事务共同提交或有明确独立失败记录后，才成为持久事实。
12. `[已确认]` 等待 A08 提交结果后，才允许向状态摘要链路发布 `ItemChange` 或成功计数。
