...
后续又观察到一次更强反例：通知使用权仍为真、主进程与 Activity 已启动、状态通知仍活跃，但覆盖安装后等待 60 秒，生产监听服务仍未自动重连。由此否定“打开应用或继续等待本身足以恢复”的假设。生产候选必须在已授权但未连接时提供正式恢复路径：可见 Activity 启动 3 秒后仍未连接则单次调用 `requestRebind()`；收到真实 `onListenerDisconnected()` 后也可延迟 3 秒请求；自动请求每进程最多 3 次且受 10 秒冷却，用户可在应用内明确点击重连但同样受冷却。禁止调用 `requestUnbind()`，禁止轮询、定时后台任务或无界重试。
