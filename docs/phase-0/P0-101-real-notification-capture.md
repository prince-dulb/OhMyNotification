...
同日监听稳定后再次拉取私有事件流，共发现 91 条 B 站来源回调、31 个唯一系统 key。去内容分解如下：

| 回调来源 | 回调数 | 唯一 key | 解释 |
|---|---:|---:|---|
| `RECOVERY_SNAPSHOT` | 90 | 30 | 同一批活动通知在三次连接中重复补扫；不是 90 条新消息 |
| `POST_OR_UPDATE` | 1 | 1 | 连接期间新到达的真实通知 |
| `REMOVED` | 0 | 0 | 尚未观察到 B 站移除回调 |

31 个唯一通知全部具有 `contentIntent` 且均为 Activity 动作；其中 30 个 key 各被三次补扫重复观察，进一步证明重连补扫必须基于系统身份幂等归并。31 个通知分布在三个 channel；标题和正文各在 30 个唯一通知中存在，另一个通知属于缺少普通标题/正文的媒体式形状。

私有可见文本只作候选分类，不升级为用户确认：28 个唯一 key 命中视频/投稿语义候选，1 个命中动态语义候选，2 个无法分类；唯一连接期间新回调属于视频语义候选。尚未观察到直播语义候选。投稿、动态、开播三类仍需用户核对实际通知身份，因此 GO 条件保持未满足。
